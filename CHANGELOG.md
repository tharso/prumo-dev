...
### Fixed
- **O nó do Inbox4Mobile: um dono para o formato, e a limitação declarada (#289)** — com o preview `stale` (índice de 25/07 contra arquivos de 30/07), os 9 itens novos viraram **uma contagem**: sem triagem, sem nomes, sem nada que sustentasse decisão. Duas causas, e nenhuma era o fallback. **Dois donos discordando:** `inbox-processing.md` mandava "mostrar apenas o link e a contagem" enquanto `briefing-montagem.md` exigia "a contagem **e a triagem**, itens numerados" — e a frase de desarme morava no módulo *outro*, sem contra-ponteiro. **Um ponteiro que nunca teria o gerador:** mandava regenerar "pelos paths de `runtime-paths.md`", mas o gerador vive dentro do pacote `prumo_runtime` e `Prumo/scripts/` não é populado por instalador nenhum — em host sem runtime o passo era impossível, e o texto não dizia isso. Agora o formato tem **um** dono, o passo de regeneração aponta o comando (`prumo inbox preview`) e declara o limite, e a degradação virou honesta: a classificação passa a ter **dois ramos** — com evidência sai `P1/P2/P3` como sempre; sem evidência (só nome e `mtime`) a prioridade sai **`não determinada`**, a ação só quando o nome a sustentar, e uma linha declara que a classificação é por nome. Os dois ramos entregam triagem **numerada**: o fallback degrada a certeza, nunca a entrega. Sem essa bifurcação o módulo proibiria inventar prioridade num passo e a exigiria no seguinte — a mesma forma do bug que ele conserta. O corte entre os ramos é **evidência material do item**, nunca frescor da fonte: metadata (nome, tipo, tamanho, `mtime`, fingerprint, URL do índice) não distingue um `IMG_1234.jpg` urgente de um print de meme. No mesmo movimento, o `ASSERT` do core parou de condicionar o link do preview à existência do **índice** e passou a exigir o **HTML utilizável** — o frescor compara mtimes e nunca conferia se o arquivo existia, então o contrato mandava linkar um fantasma. A exceção no ASSERT que o relatório propôs foi recusada: ele proíbe **abrir bruto**, não classificar — triagem por nome sempre foi permitida, e é obrigatória.
- **Cache-busting já na primeira ida, sem runtime (#291)** — em 30/07 a primeira consulta ao `VERSION` público devolveu 5.18.0 com o core em 5.78.0; o protocolo do remoto suspeito re-tentou e recebeu 5.80.0. Funcionou, e custou **duas idas à rede** no caminho que já é o mais lento. No caminho sem runtime não há cache a proteger — o agente busca uma vez por execução, sem cache agent-owned — e o CDN já mentiu em dois casos registrados. O caminho com runtime não muda: lá quem faz a rede é o CLI, com cache de 24h. **Catraca 6.753 → 6.759 (+6)**: a primeira redação custava +30 por carregar a justificativa em prosa, que foi para o `DECISIONS.md`, onde ela pertence.
...
- **A higiene ganhou o outro lado da fronteira (#261)** — a faxina detecta e para; quem resolve é a higiene, que até agora não sabia sequer detectar índice truncado. Novo check 9: refaz a conta, nomeia as fichas fora da tabela, consulta a cópia em `.prumo/backups/curated/` (#262) e oferece **restaurar preservando IDs e descrições originais**; sem cópia, avisa em voz alta que recriar dá IDs novos e descrições derivadas. Terceira saída sempre disponível: **"a remoção foi deliberada"** — e ela **grava estado**, senão o mesmo bloqueio voltaria na rodada seguinte: `<!-- lacunas-conferidas: L/S -->` (fração exata; percentual arredondado faria 101/200 e 100/200 empatarem em 50% e o crescimento passar calado) e `<!-- fichas-fora-conferidas: a.md, b.md -->` (por nome; contagem não distingue a ficha aceita fora da ficha nova de amanhã). Só o que cresce além do aceito volta a alarmar. A higiene refaz a conta com os thresholds **efetivos**, não com os defaults — senão o bombeiro chega e discorda do alarme. **Nunca reinsere sozinha** — foi exatamente o reflexo automático que teria cimentado o dano.
...
- **Gatilho na rota, com custo aprovado (#261)** — a regra 6 do core ganhou uma linha apontando a política. Política condicional sem gatilho não dispara: pra saber que precisa carregá-la, o agente teria de já conhecê-la. **Catraca 6.642 → 6.671 (+29)**, contrato novo aprovado pelo dono em duas parcelas: +10 do ponteiro e +19 do contrato de consumo (abaixo).
...
### Changed
- **O briefing passou a LER a decisão do índice (#261)** — transportar sem consumir não vale nada. O gate de capacidade do `briefing-estado.md` aceitava runtime anterior (mesmo `schema_version`, sem o bloco novo) e a família 3 da faxina mandava apenas comparar conjuntos — o ponto cego é **índice truncado sem nenhuma ficha órfã**. Agora o gate exige `indice_referencias.schema`, o frescor por fonte cobre `INDICE.md` e o manifesto de `Referencias/` (truncar depois do `prumo seed` invalida a decisão, em vez de a semente seguir "fresca" com `decisao: ok`), e a família 3 lê a decisão: `bloquear` = pendente e **proibido reindexar**, quem resolve é a higiene. Sem o bloco, o fallback confere a coluna `Arquivo` **e** a lacuna do rodapé.
...
### Changed
- **Remover do inbox nunca mais é deletar — quarentena `_to_delete/` universal (#242, P2 do épico #240)** — bug real de 27/07: no Cowork a ponte de shell proíbe deleção (`PermissionError: Operation not permitted`) e o fallback documentado mandava "solicitar a permissão do runtime; tentar novamente" — beco sem saída, com o arquivo preso no inbox pra sempre (30 itens acumulados numa rota morta desde fevereiro). Agora: (1) **máquina de remoção** por item — confirmar → registrar no REGISTRO **com o destino planejado** → mover → verificar → baixa no `_processed.json` (`reason: movido para quarentena`; o legado `fallback sem deleção física` segue válido); falha vira `REMOCAO_FALHOU`, sem retry cego; (2) destino **parametrizado pela ação**: retenção durável (`archive`/referência movem o próprio arquivo pro destino final) ≠ quarentena `_to_delete/<AAAA-MM-DD>_<escopo>/` na raiz do workspace (`discard` e original já materializado noutro canal) — em TODO host, inclusive onde `rm` funciona (decisão do dono: um contrato só, sempre reversível); (3) **`_to_delete/` é quarentena do USUÁRIO**: o Prumo nunca lista, indexa, conta em briefing/faxina/acervo, nem esvazia; colisão ganha sufixo determinístico, retry é idempotente, symlink move o link; (4) regra de sequência de arquivamento: **mover primeiro, editar depois** — nunca "escreve cópia nova, apaga a velha" (falha no meio deixava o arquivo duplicado em dois diretórios); (5) faxina segue **apenas sinalizando** processados-inconsistentes (#212 mantida) — a oferta confirmável de mover pra quarentena mora na `higiene`, **sem backup duplicado** (a quarentena já é a operação recuperável); (6) ASSERT do core recontratado 1:1 ("deletar" → "remover" — cesto neutro: rota segue **6.493**, recibo no gate). Conformidade: **C5 evoluído em vez de cenário-gêmeo** — o replay ganha op `move` (nunca sobrescreve; validação pelo pai + `lstat`, sem seguir symlink terminal), `allow_delete=False` simula a ponte do Cowork (PermissionError) e devolve o **trace** das ops; o oráculo exige quarentena datada com bytes idênticos + trilha mencionando o item + baixa no `_processed.json`, reprova **delete+recriação pelo trace** e, no negativo, reprova cópia ou marcação sem confirmação; 7 variantes pareadas, guards textuais anti-regressão (`test_quarentena_contracts.py`). Design fechado em 3 rodadas de review com o Codex antes do código.
...
### Changed
- **Decidir: o card mostra o item ou leva a ele — mostrar ≠ analisar (#191)** — report do dono ao vivo: card de NOTA pedia destino (tarefa/pauta/ideia/descartar) mostrando só meta-descrição ("sem tese explícita no preview" — o gerador despachou da **listagem** sem abrir o item); o comentário inevitável foi "preciso ver o que é". Regra nova em camadas, por custo: **nota curta → texto integral no card** (escapado); conteúdo não-elementar (imagem/vídeo/post/nota longa) → metadados + **link de visualização** (URL externa ou path **relativo** pra arquivo do workspace — o `safeUrl` já aceita); **análise pesada nunca na geração** (transcrever/OCR/resumir seguem pós-despacho, pagos só se despachados). Card que só descreve o item, sem conteúdo nem link, é inválido (checklist pré-entrega + exemplo bom/ruim de nota nos references). "Ver antes de decidir" é o **link no corpo do card**, não ação na fileira — zero round-trip. Affordance: label de `mark_seen` vira **"Marcar visto"** (o "Ver/" prometia a ação oposta), a allowlist documenta `mark_seen` ≠ `no_action` (baixa com rastro vs. encerrar sem registro), o documento ganha **legenda fixa do ⚑** ("a ação pede um detalhe no comentário") e o checklist trava **Delegar só com delegado plausível**. Waiting-for pós-delegação virou a #192. Dois rounds do Codex (PR #193) endureceram a mecânica: o texto do usuário viaja em **`conteudo_b64`** (base64 UTF-8 gerado por comando **em linha única** — `base64 | tr -d '\r\n'`, porque o GNU base64 quebra em 76 colunas e o newline dentro do literal JS é SyntaxError (round 3) — o alfabeto não carrega aspas, `\`, `<` nem newline, então quebrar ou injetar o script é **impossível por construção**; o round 2 provou que "escapar no render" não bastava: `O'Brien` quebrava o literal JS antes do render), o template decodifica e escapa (`fromB64` + `escapeHtml`, base64 inválido vira aviso), `safeUrl` **rejeita path absoluto** (`/abs`, `//server`, `file://`), o checklist pré-entrega vale pra **todo card baseado em fonte** (email exige remetente + trecho citável) e o limiar da nota curta ficou reproduzível (~400 chars, corte em fronteira de palavra). Travado por `DecidirTriagemNoEscuroGuards` + `DecidirConteudoEscapadoGuards` (13 testes TDD). Ver DECISIONS.md 2026-07-19 (#191).
...
### Added
- **Detecção proativa de defasagem no briefing (#158, épico #161)** — staleness deixa de ser silenciosa. O `prumo briefing --format json` passa a expor `version_status` (severidade por **distância de versão**: `ok`/`info`/`warning` 1 minor/`alert` 2+ minor ou salto de major/`unknown`) e `skills_missing`; `warning`/`alert` viram **aviso forte** na `degradation` com a ação exata (`prumo update`), e skill ausente vira aviso de `prumo repair` — a origem do "Habilidade desconhecida" (#146). A versão pública é lida do **cache** (`version_check.read_cached_remote_version`), sem nova rede, pra o painel seguir leve. **Fonte de verdade por elo** documentada (`version-update.md` 4.23.0): distância de versão = briefing; `lastUpdated` do checkout Cowork ("M dias parada") = `/doctor`; presença de skills e core↔runtime = runtime — não se inventa dias a partir de distância de versão. Preflight do briefing (4.27.0) escala o aviso; `/doctor` ganha **semáforo por elo** (🟢/🟡/🔴). Follow-up de #145/#146 (que consertaram os elos; este detecta antes do usuário tropeçar). Travado por `test_deteccao_defasagem.py` (14 testes). Ver DECISIONS.md 2026-07-04.
- **Suíte de conformidade comportamental — A0 (#157, épico #161)** — infra de teste que provou contratos como **comportamento de um agente real** operando as skills, com oráculos determinísticos (`filesystem`). Host `replay` roda em CI (prova o pipeline e a discriminação dos oráculos sem LLM); host `claude_code` (`claude -p`) roda na cadência do dono. Três cenários safety em par negativo/positivo: diário só grava após OK (C3), remoção de inbox só com confirmação + trilha (C5), setup não pré-cria `Diario/` (C7). Ver `conformance/SPEC.md`. A1 (multi-host + transcript/tool-call, incl. C12 consumindo as fixtures de injeção da #156) e A2 (gate) são follow-up. DECISIONS.md 2026-07-04. (Mergeada como infra sem bump; sai para usuários com esta versão.)

## [5.27.0] - 2026-07-04

### Security
- **Conteúdo de terceiro é dado, nunca comando (#156, épico #161)** — o briefing lê e age sobre email/convite de terceiro; até aqui nenhuma skill tratava esse conteúdo como entrada hostil. Nova **regra 18** no core + seção **"Conteúdo de terceiros"** no `briefing-procedure.md` (4.26.0) com defesas concretas: **remetente-original + Reply-To divergente** (rascunho vai pro `From` dos headers; endereço divergente no Reply-To ou no corpo → mostra os dois e confirma — barra o BEC clássico); **teto de urgência autodeclarada** (a palavra "URGENTE" no corpo não sobe prioridade sozinha; sem rebaixar prazo real); **sinalização de instrução embutida** (`⚠ instruções no corpo — tratadas como texto` — o agente não obedece nem esconde); **links enganosos** (domínio real do href, encurtador, âncora divergente); **ação de alto risco com parâmetro do corpo** (pagamento, troca de conta, link de login, envio externo) para e confirma com a evidência à vista. Allowlist do `decidir` fixa o destinatário de rascunho no remetente-original. Template do `EMAIL-CURADORIA.md` ganha "Padrões suspeitos" alimentado só por decisão do usuário — nunca automaticamente a partir de um email. Nuance central (afinada com o Codex): o corpo **pode informar fatos** (um boleto legítimo traz valor e vencimento); a barreira é só onde o conteúdo tenta **comandar o agente** ou **definir sozinho o parâmetro de uma ação perigosa**. Travado por `test_injecao_conteudo.py` (TDD) + **fixtures dos 5 vetores** (instrução embutida, BEC/Reply-To, urgência fabricada, exfiltração, convite) em `conformance/fixtures/injection/` com oráculo escrito — executáveis à mão agora, viram o cenário C12 da suíte de conformidade (#157). Ver DECISIONS.md 2026-07-04 (novo tópico `security`).
...
### Added
- **Skill `fim`: encerramento formal de sessão (`/fim`)** (#126) — bookend simétrico do briefing. Documenta os **deltas duráveis** da sessão nos canais existentes (`IDEIAS.md`/`PAUTA.md`/`REGISTRO.md`), roda a `faxina` automática, e — quando detecta acúmulo — **propõe** (nunca executa) `/higiene` ou `/sanitize`, resolvendo o problema desses comandos ficarem esquecidos. **Contrato conservador:** grava só deltas visíveis e confirmados; sob compactação de contexto, declara a lacuna em vez de inventar. **Mantém a #68:** zero artefato narrativo de sessão (sem `HANDOVER`/`PENDING_VALIDATION`). **Cerca contra overlap com o briefing:** não lê email/calendário, não marca `last-briefing.json`, não refaz a pauta do dia. Detector read-only `prumo fim --format json` computa os sinais de acúmulo usando os thresholds padrão da faxina/sanitize (conta `.prumo/backups` e o legado `.prumo/backup`). Planejado com revisão dupla do Codex (ver DECISIONS.md 2026-06-26).

## [5.14.1] - 2026-06-26

### Fixed
- **Correções de segurança/robustez no `acervo` (review de código do Codex)** (#125) — (1) **perda de dados em lote:** o `apply` enumerava o limbo uma vez para o lote, então um segundo `delete` no mesmo arquivo usava linhas já deslocadas pelo primeiro e cortava o trecho errado — agora **re-enumera antes de cada delete**; (2) a remoção **recomputa o hash normalizado do trecho** imediatamente antes de cortar (fonte única `fragment_content_hash`, compartilhada com o enumerador); (3) **XSS:** os itens passam a ser injetados num bloco `<script type="application/json">` via `prumo acervo --format html-items`, que escapa `<`/`>`/`&` — um título com `</script>` não fecha mais a tag; (4) arquivar referência usa **nome único** na quarentena (não sobrescreve um arquivo já arquivado); (5) `include_pauta` **normaliza** título/comentário para uma linha (comentário com newline não injeta heading/bullet na PAUTA); (6) ordenação por idade não repete mais headings de grupo. +8 testes de regressão (2 deletes no mesmo arquivo, colisão de nome incl. contador, injeção por newline em título e comentário, prova de ordem registro→remoção, XSS).

## [5.14.0] - 2026-06-26

### Added
- **Skill `acervo`: navegador HTML do limbo** (#125) — interface offline pra navegar e revisitar o que ficou parado: ideias soltas (`IDEIAS.md`), itens da seção `Hibernando` da `PAUTA.md` e referências guardadas (`Referencias/`, exceto as operacionais `INDICE.md`/`EMAIL-CURADORIA.md`/`WORKFLOWS.md`). Forka a mecânica verificada do `decidir` (offline, fonte local, bloco JSON `prumo_acervo_report.v1`, clipboard), mas troca o despacho da pauta por **navegação** (busca, filtro por grupo, ordenação por "mais parado primeiro") + **3 verbos fixos**: incluir na pauta, atacar agora, excluir. Enumerador read-only `prumo acervo --format json` (parsing determinístico de markdown local — mantém a #104). Execução segura `prumo acervo --workspace <ws> apply`: cada item carrega **proveniência** (path/âncora/linhas/hash) e **excluir arquiva, não apaga** (quarentena `Prumo/Arquivo/Acervo/` + registro no `REGISTRO.md`), bloqueando remoção quando o hash diverge, o item é ambíguo, o path escapa de `Prumo/` ou é referência operacional. Artefatos efêmeros (HTML + fonte copiada) limpos pelo `sanitize`. Planejado com revisão dupla do Codex (ver DECISIONS.md 2026-06-26).
...
- **Comandos shell anunciados aos hosts agora batem com o parser** (#64 follow-up, audit Codex em #81 P1.2) — `--refresh-snapshot` foi removido de todos os lugares (4 templates/payloads + assinatura de `build_briefing_payload`). A flag foi removida do parser em algum momento mas continuou sendo anunciada, e o parâmetro `refresh_snapshot` em `build_briefing_payload` era dead code (recebido e nunca usado). Skill `prumo:abrir` corrigida pra usar `prumo` puro (root command não aceita `--workspace`). Placas antigas em 4 lugares (`render_wrapper_merge_block` em `workspace.py`, fallback chain em `agent-md-template.md`, texto interno de `briefing/SKILL.md` e `start/SKILL.md`) atualizadas pra apontar `prumo:abrir` como rota primária.
- **Skill `higiene` referenciava módulo inexistente** (`perfil-hygiene.md`) — corrigido para `claude-hygiene.md`, que é o módulo real (cita audit Codex em #81 P2.5).
- **Drift de versão em manifestos distribuídos e headers de skills canônicas** (#83, audit Codex em #81 P1.3) — 6 arquivos estavam declarando `5.1.1` enquanto runtime, `pyproject.toml`, `VERSION` e `plugin.json` raiz já estavam em `5.2.0`. Bumpados: `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `.codex-plugin/plugin.json`, `skills/prumo/references/prumo-core.md` (header `prumo_version`), `skills/prumo/references/modules/dispatch.md` e `skills/prumo/references/modules/load-policy.md` (headers `module_version`). `test_version_sync.py` expandido para cobrir todos os 10 lugares com `version`/`__version__` (mais teste dedicado documentando a exceção do `.codex-plugin/marketplace.json`, que não tem campo por design do schema do Codex). CI agora quebra automaticamente se algum manifesto sair de sincronia.
- **Wheel publicado agora carrega skills, plugin.json e VERSION** (#82, audit Codex em #81 P1.1) — antes, o wheel empacotava só `runtime/prumo_runtime/`, e instalações via `pip install` ou `curl | bash` produziam runtime órfão. `prumo setup` levantava `TypeError` porque `install_skills()` fazia `repo / "skills"` quando `repo_root_from()` retornava `None`. Fix em três frentes: (1) `pyproject.toml` ganhou `[tool.hatch.build.targets.wheel.force-include]` mapeando `skills/`, `plugin.json` e `VERSION` para `prumo_runtime/_bundled/` dentro do package; (2) `repo_root_from()` ganhou fallback que aponta para `_bundled/` quando subir a árvore não encontra repo real (caso típico de wheel em `site-packages/`); (3) `install_skills()` trata `repo is None` graciosamente com retorno vazio em vez de crashar. Todos os call sites de `repo_root_from()` (em `templates.py`, `briefing.py`, `start.py`, `inbox_preview.py`) automaticamente passam a funcionar em wheel install sem mudanças individuais. Adicionado job CI `wheel-install-smoke` que constrói wheel, instala em venv limpo, roda `prumo setup` e verifica que `.prumo/skills/abrir/SKILL.md` + `.prumo/system/PRUMO-CORE.md` (com `prumo_version: 5.2.0`) materializam corretamente — defesa permanente contra essa categoria de bug. Validado manualmente em venv Python 3.11 antes do commit.
- **sdist tarball agora contém os assets necessários pra reconstruir o wheel** (#82 follow-up apontado por Codex) — o `force-include` do wheel é resolvido em build time a partir do CWD, mas como `[tool.hatch.build.targets.sdist].include` não listava `skills/` nem `plugin.json`, o sdist saía sem esses arquivos e tentar reconstruir wheel a partir do sdist extraído falhava com `FileNotFoundError: Forced include not found`. Adicionado `/skills` e `/plugin.json` ao include do sdist. Job `wheel-install-smoke` ganhou steps que (a) validam `prumo start --format json` e `prumo briefing --format json` retornando `canonical_refs` apontando pro `_bundled/` (subindo o sarrafo de "produto não crash" para "produto não empobrecido"), e (b) fazem roundtrip sdist → wheel verificando que o wheel reconstruído contém o bundle.

### Added
- **Skill `prumo:abrir`** ([commit d213cfe]) — porta de entrada para invocação curta. Curta de propósito: redirect pro módulo `dispatch.md` que já existia. Resolve #64 sem mexer no namespace de core (alternativa B, renomear `skills/prumo/` pra `skills/setup/`, foi descartada por arrastar `references/`, `references/modules/` junto, ficando incoerente — `briefing` lendo módulo canônico de dentro de pasta chamada `setup`).
- **Teste `test_announced_commands.py`** — valida que todos os comandos shell anunciados aos hosts (via templates, payloads, skills) existem de fato no parser do CLI. Defesa contra a categoria de bug pego pelo Codex em #81 P1.2 (templates/payloads anunciando flags ou comandos que o parser não aceita).
