...
### Added
- **Medidor de execução do briefing (#284)** — `scripts/briefing_transcript_audit.py` lê o transcript e devolve sempre a mesma tabela, com `time_to_first_time` no centro: quanto tempo até o usuário ver a primeira coisa útil. Era a promessa da #196 e a única métrica que ninguém media — a catraca `briefing_f0f1_words` mede palavras de entrada carregadas antes do primeiro dado, e é cega para turnos, saída e relógio. O detector reprova com `first_time_not_emitted_before_external_channels` quando um canal externo abre antes da entrega, e **texto em `thinking` não conta como entrega** — compor não é entregar. Quatro travas de desenho: schema desconhecido vira `unsupported` em vez de número bonito; só agregados saem (nunca prompt, email ou resultado de ferramenta); canal externo casa por **sufixo do método**, porque o prefixo do servidor MCP é um UUID que muda por instalação; e nada disso vira gate de performance no CI, que testa apenas o parser contra fixtures sintéticas.
