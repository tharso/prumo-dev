...
> Se você está aqui, ótimo. Mas o volante mesmo está em `Prumo/AGENT.md`.
...
1. **Perímetro automático:** por iniciativa própria, opere apenas nos caminhos do mapa do workspace em `Prumo/AGENT.md`, somados aos declarados em `Prumo/Agente/MAPA-AUTORAL.md` (leia-o na abertura, quando existir). Zero exploração espontânea da raiz — **inclusive na descoberta**: as pastas do Prumo são sempre `Prumo/` e `.prumo/`; vá direto a elas, nunca liste a raiz pra "descobrir o workspace".
...
3. Contexto vivo do usuário mora em `Prumo/Agente/`.
