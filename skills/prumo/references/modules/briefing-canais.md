...
**Busca além da cauda (Codex, 325-r1):** a cauda do REGISTRO na semente tem ~10 linhas — evento de reabertura fora dela NÃO pode suprimir o item de volta em silêncio (a promessa é vivo **até despacho**, não até a cauda girar). Havendo item suprimido SEM evento na cauda, buscar *"Reabertura:"* **junto do identificador do item suprimido** no `REGISTRO.md` por **recorte dirigido** (grep/leitura delimitada — nunca o arquivo integral, e nunca o prefixo sozinho, que num REGISTRO cheio devolveria as reaberturas de TODOS os itens): as linhas do item respondem a validade E a lista de identidades já usadas. O gatilho no mapa cobre a variante zero-canais: suprimido sem evento na cauda carrega este módulo mesmo sem email, agenda ou inbox (Codex, 325-r2).
