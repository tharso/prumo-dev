...
**Busca além da cauda (Codex, 325-r1):** a cauda do REGISTRO na semente tem ~10 linhas — evento de reabertura fora dela NÃO pode suprimir o item de volta em silêncio (a promessa é vivo **até despacho**, não até a cauda girar). Havendo item suprimido SEM evento na cauda, buscar *"Reabertura:"* no `REGISTRO.md` por **recorte dirigido** (grep/leitura delimitada — nunca o arquivo integral, regra do recorte): os eventos do item respondem a validade E a lista de evidências já usadas.

**Evidência consumida não re-queima (Codex, 325-r1):** o evento registra a evidência — candidata que **já consta em evento de reabertura anterior do MESMO item** está CONSUMIDA e não reabre de novo; **só evidência nova reabre**. Sem esta regra, o email ainda dentro da janela de 24h (ou o evento ainda na agenda, ou o arquivo ainda no inbox) re-queimaria a data nova no briefing seguinte — e o re-suprimir de UM passo viraria zero passos.
