...
- Caixas declaradas (#245): se o usuário marcou pastas como `(caixa de entrada)` no mapa autoral, apresentar a contagem em uma linha — "Caixas declaradas: N em `X/`, M em `Y/`" —, sem despejar itens. Contar e cobrar, nunca reorganizar. Com N > 0, a linha fecha com a oferta de catalogação (#332) em uma frase — oferta na apresentação, nunca ação sem confirmação (o comportamento é do `briefing-canais.md`; aqui só o formato).
