...
**A checagem de faxina declara o resultado SEMPRE (#217 — verificável, não pulável):** a linha obrigatória do primeiro tempo (formato na montagem). O **contrato mínimo da checagem mora AQUI** (sem circularidade: checar não exige abrir o módulo executor); `faxina.md` carrega só quando alguma família está PENDENTE, pra executar. Os NÚMEROS vêm do bloco `faxina.thresholds` da semente (#258) — **efetivos**, com o override do usuário (`Prumo/Custom/rules/faxina-thresholds.md`) já aplicado pelo runtime; `thresholds_source` diz se veio de default ou override, e `ignored_keys` lista o que o override trazia fora do vocabulário (reportar ao usuário, nunca adivinhar). **Sem semente** (ou com `schema` de thresholds desconhecido): carregar `faxina-thresholds.md` e usar os defaults — é o único caminho em que o doc entra na rota. A cláusula antiga de "recalcular quando o efetivo divergir do declarado" morreu: a semente não declara mais um número e conta por outro. **"Nada pendente" só depois de olhar as CINCO famílias do `faxina.md`** — as cinco checagens baratas:
