...
- **Sem runtime:** buscar o `VERSION` público (`raw.githubusercontent.com/tharso/prumo/main/VERSION`) via WebFetch/`curl` — sem cache agent-owned (o agente não escreve estado fingindo ser runtime, #214).
- Não parar no drift local: "comparar só o core do workspace contra si mesmo" não é a checagem de versão.
- **Remoto MENOR que o local é resposta SUSPEITA (#215)** — cache de CDN mentiu no caso real (5.18 servido quando o real era 5.49). Nunca ler como "em dia": seguir o protocolo de suspeita do `version-update.md` (cache-busting → persiste menor = status **desconhecido**, declarado em uma linha).
