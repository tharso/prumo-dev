...
- **Remoto MENOR que qualquer versão local confiável é resposta SUSPEITA — nunca "em dia" (#215):** o **dono do protocolo é o `version-preflight.md`** (#180 — este módulo pode nem estar carregado quando o caso acontece; o preflight roda sempre). Resumo do que o preflight manda: cache-busting uma vez → persistindo menor, status **desconhecido** declarado em uma linha → seguir com a comparação LOCAL (bundle × workspace), que continua válida. Caso real de 25/07: WebFetch devolveu `5.18.0` com o público em `5.49.0` (~30 versões stale). **Nunca** ler "remoto menor" como "estou em dia".
