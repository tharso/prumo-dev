...
2. Responder em blocos curtos numerados continuamente, **cada elo com semáforo** (`🟢 ok` / `🟡 defasado` / `🔴 quebrado`) na frente, pra a saúde saltar aos olhos:
   ...
   - `Sessão (conta)` — o que a conta materializa nas sessões (`session_materializations`, camada 5 da propagação, #190): `🟢` sem divergência; `🔴` se `session_divergence` (materializada atrás do marketplace — possível registro server-side congelado; `session_note` traz o `updatedAt` e `session_action` a receita validada: re-add como `owner/repo`, nunca URL raw, testar em sessão nova). Store legada presente (`legacy_stores`) entra aqui como `🟡` com a nota de entulho removível.
