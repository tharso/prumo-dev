...
1. **Primeiro o local, sem esperar o externo:** leituras locais mínimas (a PAUTA via `local_panorama.pauta` da semente — arquivo `PAUTA.md` só no fallback do Passo 3 —, `PERFIL.md`, `PESSOAS.md`, `EMAIL-CURADORIA.md`) — o **primeiro tempo do Passo 5 sai daqui, antes de qualquer resultado de email/calendário**. Na sequência, os canais externos em ordem de prioridade: metadata do Gmail (Camadas 1 e 2) → Calendar MCP → listagem plana de `Inbox4Mobile/` quando não veio na semente. **Paralelismo por subagente: DESLIGADO por default** — só habilitar num canal comprovadamente lento com ganho líquido demonstrado (#205: spawn ~3s; 22–28s por chamada de metadata medidos).
