...
| `.prumo/state/rascunho/` | **Território do agente (#263).** Subterritório declarado dentro de `state/` — **estende** a #214 (o agente não escreve estado do runtime) em vez de revogá-la: aqui nada é fonte de verdade, tudo é descartável e nada é autoral. Criado LAZY, no primeiro uso; o setup não pré-cria e ausência = zero candidatos. Varrido pela `sanitize` (família `agente_rascunho`, `move-to-backup` depois de `ephemeral_days`), nunca por `rm` — funciona sob a ponte do Cowork. |
