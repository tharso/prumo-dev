...
## 2026-07-29 — No layout antigo (flat) a leitura funciona e a escrita manda migrar (#268)
...
**Decisão (dono, 29/07):** no flat, **leitura funciona e escrita para**. O dry-run da `sanitize`, o status de versão do core e o panorama passam a operar normalmente; `sanitize --apply`, `prumo seed` e o snapshot dos curados param com uma linha que oferece `prumo migrate`. A trava do snapshot mora dentro de `snapshot_curated()`, não nos dois chamadores, pra que ritual futuro que peça snapshot já nasça protegido. O repair pós-update não roda no flat.
