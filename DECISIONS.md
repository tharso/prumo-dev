...
**Decisão (dono, 11/08):** contagem > 0 → a linha fecha com a **oferta de despacho em uma frase**. Oferta, nunca ação: sem confirmação explícita, nenhuma escrita e nenhuma leitura de conteúdo da caixa. Com confirmação, lote único pelo fluxo existente do `inbox-processing.md` (destino resolvido pela precedência #243 antes da oferta, com a copy nomeando o destino; na Biblioteca: convenção canônica de nome, frontmatter preservado + `tipo`/`origem` adicionados, `INDICE.md` via lock com a Descrição nascendo do motivo; em destino de contrato autoral: como o contrato mandar, sem convenção nem indexação da Biblioteca; trilha no REGISTRO antes da remoção); item sem frontmatter legível, sem motivo inferível ou dado, duplicado ou ambíguo degrada pra proposta individual. Pro workspace do dono, o modelo aprovado foi o C da investigação: `Clippings/` segue caixa de entrada transitória, o conteúdo migra com frontmatter preservado e a `.base` do Obsidian filtra por tag (consultando entrada e biblioteca juntas — o desenho de 27/07 do `Escrita/README.md`, agora completo).
