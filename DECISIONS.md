...
## 2026-08-11 — Catraca aperta pro medido: largest_file 868 → 867, rota 6806 → 6805

**Tópicos:** governance, code-quality.
**Issues relacionadas:** nenhuma (aperto direto do dono no chat de 11/08; origem das melhoras: #301 — extração do `update_sources.py` — e #325 — cesto fechou um abaixo do teto).
**Relações com decisões anteriores:** **mantém** 2026-08-03 (#322) — o teto 6806 foi contrato aprovado e não muda de natureza: a catraca anda pra baixo a partir do MEDIDO, e medido era 6805 desde o merge da #325; **mantém** 2026-05-06 (quality-gate) e o padrão dos apertos anteriores (2026-07-29 coverage 85→87): aperto, não recalibração. Nenhuma revogação identificada após consulta ao índice temático.

**Contexto:** as duas métricas mediam melhor que o baseline desde 03/08, e baseline atrás do medido permite regredir 1 ponto em silêncio. O Codex relembrou a pendência em reviews sucessivos; a regra da casa é o agente propor com valores lado a lado e o dono decidir.

**Decisão (dono, 11/08):** apertar as duas — `largest_file_lines` 868 → 867, `briefing_f0f1_words` 6806 → 6805.

**Alternativas consideradas:** apertar só `largest_file` (deixaria 1 palavra de folga na rota — recusada pelo dono); deixar como está (recusada — margem parada é regressão esperando passar).

---

## 2026-08-11 — A última milha do workspace: update declara o que não sincronizou; doctor mede o elo do checkout (#335)

**Tópicos:** distribution, governance.
**Issues relacionadas:** #335 (executa — spec consensada em 4 rounds de spec review com o Codex, sessão `019ff2eb-216c-7190-ad9a-d18b5dd7cdab`; o dono delegou o recorte ao consenso Claude↔Codex no chat de 11/08, com o merge pausado pro OK explícito dele), #299 (vizinha — INTACTA: conteúdo/digest bundle × checkout; `workspace.skills` fica reservado pra ela), #324 (padrão do achado nomeado), #146/#170 (elos vizinhos do update), #268 (guardas do flat), #272 (repair manual no flat segue lá).
**Relações com decisões anteriores:** **mantém** 2026-07-05 (#108) e 2026-05-04 (#77) — o runtime segue o único canal de update de `.prumo/skills/`; a pergunta da issue ("skills são canal exclusivo do bundle do app?") foi respondida SEM revogar nada. **Mantém** 2026-04-22 (workspace-first/stateless) — é o fundamento da rejeição da opção A: "o Prumo não memoriza workspaces entre sessões"; fora do CWD, nenhum ator sincroniza automaticamente — o produto garante VISIBILIDADE, não sincronização. **Estende** 2026-06-24/#146 — o pós-update ganha superfície honesta quando não pode agir. **Vizinha** de 2026-07-05 (#170) — comparações distintas e ambas vivas: o `--check` mede core × versão pública remota; o doctor mede core × runtime EM EXECUÇÃO. **Mantém** 2026-07-29 (#268) — precedência do flat preservada; o doctor aponta `migrate`. **Estende** 2026-08-03 (#324) — o padrão "última milha congelada vira achado nomeado" vai da store pro workspace (`workspace.core` aditivo no schema `prumo_doctor_host.v1`). Nenhuma revogação identificada após consulta ao índice temático.

**Contexto (o caso-mãe):** 11/08, `prumo update --yes` rodado fora do workspace: runtime foi a 5.95.0 e o checkout embarcado do DailyLife ficou em 5.94.0 — em silêncio; sessões cloud do Cowork leem o checkout do workspace (provado por sonda na sessão). Mesma classe do incidente de 05/08 no REGISTRO. O spec review achou o agravante no código: o warning de binário divergente só existia no JSON (`_emit` nunca o imprimia) e `repair_suggested` era setado ANTES da guarda — o texto recomendava exatamente o repair que sincronizaria o workspace com a fonte errada.

**Decisão (recorte B + C-lite da issue; opção A rejeitada):** (B) update bem-sucedido fora de workspace declara `post_update.last_mile` em texto e JSON, com comando copiável (`prumo repair --workspace /caminho/do/workspace`) apenas quando a versão pós-update confirmou; invariante: versão não confirmada ⇒ ZERO recomendação de repair na saída inteira (inclusive a linha do core do #170), com recuperação POR CAUSA — a guarda do interpretador (`sys.executable`) orienta verificar/reinstalar o runtime no interpretador atual sem mencionar PATH; a guarda do PATH (`shutil.which`) orienta expor (passo 0) ou alinhar o PATH, e a nota específica SUBSTITUI a recomendação genérica. (C-lite) `doctor --host` ganha o elo do workspace: `prumo_version` do core × runtime que executa o diagnóstico, estados fechados (`em_dia` por igualdade de tupla numérica; `divergente` com direção preservada; ausente/vazio/malformado ⇒ `indeterminado`, nunca direção inventada; flat tem precedência ⇒ `legacy_flat` + `migrate`, sem bloco `core`), e **coerência régua × fonte**: `recommended_command` só sai quando o `prumo` do PATH coincide em versão com o runtime em execução — senão o ACHADO sai e a prescrição é suprimida, com `path_runtime_version` visível no JSON. Emenda do code review (Codex, 335-code-r1): com `core_ahead_of_runtime` a prescrição é `prumo update` — nunca `repair`, que convergiria o workspace PRA BAIXO, pro runtime velho que o próprio achado denuncia.

**Alternativas consideradas:** opção A — update alcança workspaces fora do cwd via registro ou flag (rejeitada: registro viola o stateless de 2026-04-22; a flag duplicaria `prumo repair --workspace` e não teria evitado o incidente, que usou `prumo update --yes` cru); "lembrar o último workspace reparado" (rejeitada: não há fonte legítima sem estado novo); varrer disco/`$HOME` atrás de workspaces (rejeitada: descoberta especulativa); digest de conteúdo no doctor (fica na #299 — metadado de versão é o proxy assumido desta entrada); bump do schema pra v2 (rejeitado: bloco aditivo, precedente da #324).

---

## 2026-08-11 — A contagem de caixa declarada ganha saída: oferta de catalogação com confirmação (#332)

**Tópicos:** briefing, workspace-layout, governance.
**Issues relacionadas:** #332 (executa — nasceu da investigação Clippings inbox × biblioteca de 11/08, com decisão do dono no chat).
**Relações com decisões anteriores:** **estende** 2026-07-27 (#245) — o inventário ("contar e cobrar") ganha a oferta de despacho; o escopo do marcador fica INTACTO: sem processamento automático, sem ledger, e a leitura de conteúdo continua proibida ANTES da confirmação. **Mantém** 2026-07-27 (#242) — o lote roda pela máquina de remoção, com trilha antes. **Mantém** 2026-07-27 (#243) — o destino do lote é resolvido pela escala de precedência ANTES da oferta (a oferta é o caso concreto que abre o contrato autoral; qualquer degrau pode resolver), e a copy nomeia o destino resolvido; sem degrau aplicável vale o destino que o fluxo de material de referência do `inbox-processing.md` já usava (`Referencias/`) — nenhum default novo de organização é criado. O corte do pacote é o **destino resolvido**, não a fonte: `Referencias/` recebe o pacote da Biblioteca qualquer que seja o degrau; qualquer outro destino segue a regra da fonte vencedora, sem convenção nem indexação da Biblioteca. Do `inbox-processing.md`, **só a máquina de remoção é reutilizada** (confirmar → registrar → mover → verificar) — destino e pacote vêm da seção da oferta, nunca do destino fixo do §Material de referência (Codex, 332-r2/r3/r4). **Mantém** 2026-07-27 (#244) — `INDICE.md` só via lock atômico. **Mantém** 2026-08-03 (#305) — a renomeação do lote é na convenção canônica `Autor_Assunto_AAAA-MM-DD.<ext>`, mantendo o arquivo na rota de recuperação do índice (Codex, 332-r1). **Mantém** 2026-07-02 (#140, fichário) — `keep_with_reason` vale item a item também no lote (confirmação genérica não compra o motivo; Codex, 332-r1), e a `ficha-de-fonte.md` segue dona do mapeamento do índice: a Descrição nasce do motivo de retenção, a `description` do clipper complementa, nunca substitui (Codex, 332-r2 — a spec da issue dizia o contrário e foi emendada por comentário corretivo publicado na issue em 11/08: contrato canônico vence spec). Nenhuma revogação identificada após consulta ao índice temático.

**Contexto:** a #245 fez o briefing contar o que apodrece — e parar na contagem. Medido no workspace real: 5 artigos parados em `Clippings/` de 27/06 a 11/08, e mais 11 chegando no dia em que a solução foi desenhada. A investigação de 11/08 expôs a raiz: o produto só tinha dois formatos de pasta autoral (caixa contada que espera esvaziar, ou pasta muda), e a contagem sem saída vira ruído que o usuário aprende a ignorar.

**Decisão (dono, 11/08):** contagem > 0 → a linha fecha com a **oferta de despacho em uma frase**. Oferta, nunca ação: sem confirmação explícita, nenhuma escrita e nenhuma leitura de conteúdo da caixa. Com confirmação, lote único pela máquina de remoção do `inbox-processing.md` — só a máquina; destino e pacote vêm da seção da oferta (destino resolvido pela precedência #243 antes da oferta, copy nomeando o destino; corte por destino: em `Referencias/`, pacote da Biblioteca — convenção canônica de nome, frontmatter preservado + `tipo`/`origem` adicionados, `INDICE.md` via lock com a Descrição nascendo do motivo; em qualquer outro destino, a regra da fonte vencedora, sem convenção nem indexação da Biblioteca; trilha no REGISTRO antes da remoção); item sem frontmatter legível, duplicado, ambíguo — ou, no destino Biblioteca, sem motivo — degrada pra proposta individual. Pro workspace do dono, o modelo aprovado foi o C da investigação: `Clippings/` segue caixa de entrada transitória, o conteúdo migra com frontmatter preservado e a `.base` do Obsidian filtra por tag (consultando entrada e biblioteca juntas — o desenho de 27/07 do `Escrita/README.md`, agora completo).

**Alternativas consideradas:** marcador novo `(biblioteca)` com fichas-ponteiro e contagem de não-catalogados (modelo B) → rejeitado pelo dono nesta rodada — preferiu completar o desenho existente a criar vocabulário novo; full-auto no fechamento do briefing (v2) → adiado por decisão explícita: revisitaria o "nenhum processamento automático" do #245 e exigiria ledger — se vier, é decisão nova neste arquivo, depois da v1 provar o mecanismo em campo.

---

## 2026-08-03 — Evidência nova reabre item suprimido por `cobrar:` — a soneca que qualquer batida cancela (#325)

**Tópicos:** briefing, governance.
**Issues relacionadas:** #325 (executa — as quatro regras vieram de grill de domínio com o dono no chat de 03/08, uma pergunta por vez; **origem** da regra), relatório de campo da rodada 18h (origem do incidente, atrito nº 7 — o mais grave), #237 (vizinha — `cobrar:` sem data; independente), #238 (padrão da fonte durável), #242 (máquina de escrita da PAUTA).
**Relações com decisões anteriores:** **estende** o filtro de cobrança (formalizado nos módulos desde a #195/#180) com a primeira exceção regrada — e a paridade do runtime fica INTACTA: `visible_today` da semente segue calculando a regra base, a exceção é do agente e aplica-se DEPOIS, sobre os eventos de reabertura do REGISTRO (cauda primeiro; busca dirigida além dela quando o suprimido não tem evento VÁLIDO na cauda — stale visível não dispensa). **Mantém** 2026-07-02 (#141) e o padrão da #238 — evento durável no REGISTRO, sem marker novo na PAUTA. **Mantém** 2026-07-27 (#242) — a linha da PAUTA só muda via despacho; o briefing nunca mexe no `cobrar` por conta própria. Nenhuma revogação identificada após consulta ao índice temático.

**Contexto (o caso-mãe):** ganho de capital de Santos (~R$ 20,2 mil, multa no teto) e processo Santa Maria (preparo a recolher, apelação viva) suprimidos por `cobrar: 06/08` — e a guia de custas do processo chegou por email às 11h12 DO MESMO DIA, virando o item 26 do panorama sem nenhuma conexão com a pendência escondida. O filtro estava certo pela letra; o resultado escondeu o grave no dia em que a evidência gritou.

**Decisão (dono, grill de 03/08 — quatro respostas):** (1) **reentrada universal** — qualquer evidência nova sobre item suprimido reabre, sem juízo de relevância no filtro; (2) **a evidência queima a supressão** — o item volta VIVO e permanece até despacho; `cobrar` é soneca, não cofre; (3) **os três braços cruzam** — email, calendário e inbox verificam contra a lista de suprimidos (da semente: `cobrar.state == future`, zero leitura nova); (4) **na dúvida, reabre** — vínculo por julgamento do agente; ambíguo entra como "vínculo possível", nunca descartado em silêncio. Apresentação com o vínculo visível (evidência + data original do cobrar) no bullet das pendências.

**Mecânica:** evento de reabertura no REGISTRO no fechamento, carregando a **data original do cobrar** — o evento vale enquanto o cobrar da linha for o MESMO registrado: re-suprimir é UM passo no despacho (data nova → o evento envelhece sozinho, sem baixa nem ledger novo); resolver/descartar remove a linha e não sobra o que reabrir. Onze endurecimentos do review (Codex, 325-r1 a r4): reabertura descoberta por email/agenda apresenta no SEGUNDO tempo (no fluxo de dois tempos, Gmail/Calendar só abrem depois da primeira entrega — sem o caminho, a descoberta só apareceria amanhã); a cauda da semente tem ~10 linhas, então suprimido sem evento na cauda exige busca dirigida por "Reabertura:" no REGISTRO (recorte, nunca integral) — evento fora da cauda não suprime de volta em silêncio; evidência já registrada em reabertura anterior está CONSUMIDA — só evidência nova reabre, senão o email ainda na janela re-queimaria a data nova e o re-suprimir viraria zero passos; o evento carrega IDENTIDADE ESTÁVEL obrigatória (ID de mensagem/thread, ID de ocorrência do calendário, caminho do arquivo — o consumo compara pelo id, nunca pelo resumo: o mesmo email com outras palavras parecia novo); e o gatilho do canais no mapa ganhou a perna do suprimido sem evento na cauda — na variante zero-canais F2 nunca abriria e a reabertura fora da cauda ficaria suprimida com briefing completo. A busca leva o identificador do ITEM, nunca o prefixo sozinho — com teto explícito (20 linhas mais recentes do item; corte declarado como 'histórico truncado'). Do r3: o gatilho exige evento VÁLIDO na cauda (evento stale não dispensa a busca — um consumido fora dela pareceria novo e re-queimaria), e a identidade do ITEM é a SEÇÃO da PAUTA + a linha sem os markers e sem numeração — a chave nunca inclui o marker que a re-supressão altera, a seção discrimina texto igual em seções distintas, e colisão na mesma seção trata pro lado de mostrar. Do r4: com histórico truncado o consumo é INCOMPLETO por construção — candidata sem match nos ids visíveis reabre como 'vínculo possível — histórico truncado', nunca fica suprimida (na dúvida do histórico vale a régua do dono: mostrar custa um clique, esconder custa o incidente-mãe). A exceção de F1 coube DENTRO do orçamento — a reescrita compacta do filtro e a poda do transporte 2 pagaram tudo com troco: **cesto fecha em 6805, um abaixo do teto 6806**. Guards em `test_reentrada_evidencia.py` com asserts de POSIÇÃO (exceção DENTRO do filtro; vínculo DENTRO do bullet) e da relação de staleness — a mutação barata seria "valer até baixa explícita", recriando o ledger que o desenho evitou.

**Validação de campo pendente (critério 1 da issue):** o replay do caso real — PAUTA de 03/08 + email da guia de custas → Santa Maria reaberto e vinculado — se prova no próximo briefing real (smoke manual, regra do CLAUDE.md pra mudança de comportamento).

**Alternativas consideradas:** filtro com juízo de relevância (rejeitada pelo dono no grill — pergunta 1: falha pro lado de mostrar); reabertura valendo só no dia da evidência (rejeitada — pergunta 2: se o dono não viu o briefing do dia, o incidente se repete); só o braço de email (rejeitada — pergunta 3: cobertura completa); só vínculo claro (rejeitada — pergunta 4: o caso sutil escapar é a espécie de erro que motivou a regra); marker `reaberto:` na linha da PAUTA escrito pelo briefing (rejeitada — #242: briefing não muta pauta; e o evento no REGISTRO com staleness pela data dá a mesma persistência sem escrita nova).
