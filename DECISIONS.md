...
**Decisão:** rodapé como **sugestão** + **sonda** do candidato antes de qualquer escrita; concorrência por `mkdir` da folha (sem `-p`, com os pais criados idempotentemente antes) ou `O_CREAT|O_EXCL`; **sem primitiva disponível, não escreve**; liberação **move, nunca deleta** (#242 — `rmdir` travaria o índice pra sempre no host que proíbe deleção), com a regra de colisão da quarentena; **sem retomada automática por idade** (heurística reintroduz a corrida que o lock existe pra matar).
